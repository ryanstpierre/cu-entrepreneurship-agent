...
const CUEntrepreneurshipAgent = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      // Support both local dev (http://localhost:8787) and deployed production
      const isDev = window.location.hostname === 'localhost'
      const API_URL = isDev ? 'http://localhost:8787' : 'https://cu-entrepreneurship-agent-production.ryan-c9e.workers.dev'
